          ...
          <div class="card-body table-responsive">
              <table id="tbResults" width="100%" class="table table-bordered table-hover display nowrap" cellspacing="0">
                  <thead>
                    ...
                    <th scope="col">{!! trans($lang.'result_table.col_info') !!}</th>
...
@slot('modal_body')
        <div class="form-group">
            <div class="row col-md-12" id="content">
                ...
                {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("input_group","info",$lang.'form.register.lbl_info_suscriber',true,"col-md-12") !!}
            </div>
        </div>
                <div class="row">
...
<script>
    ...
    function SaveData(){
        ...
        @slot('app_group',$group)
                ...
                @slot('ws_name','suscriber-register')
                @slot('parameters'," 'id': id, 'email': email, 'info_suscriber': info " )
                @slot('result_success')
    ...
    function LoadDataId(id){
        @component(config($group.'.ui.component.engine.ajax.view'))
            ...
            @slot('ws_group','entity')
            @slot('ws_name','suscriber-get')
            @slot('parameters', " 'suscriber_id':id ")
            @slot('result_success')
    ...
    function clearInput() {
        ...
        $("#btn_save").html("{{trans($lang.'btn_register')}}");
    }
</script>
<!--scrits-->
