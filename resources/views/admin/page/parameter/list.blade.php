                    ...
                    <div class="form-group">
                        <div class="row">
                            <div id="selects" class="row col-md-12">
                                {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("select","code",$lang.'form.filters.lbl_parameter',true,"col-md-12","fas fa-envelope") !!}
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12" style="text-align: right!important;">
                            <button class="btn btn-info col-md-2" style="margin: 5px!important;" id="btn_search" onclick='resetResults()'>{!! trans($lang.'btn_reset') !!}</button>     
                            <button class="btn btn-success col-md-2" style="margin: 5px!important;" id="btn_search" onclick='loadDataById()'>{!! trans($lang.'btn_search') !!}</button>     
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function loadData(){
            @component(config($group.'.ui.component.engine.ajax.view'))
                ...
                @slot('ws_name','parameter-get-codes')
                @slot('parameters', "")
                @slot('result_success');                         
                    $('#code').html('');
                    let query="";
        ...
        function loadDataById(){
            ...
            @component(config($group.'.ui.component.engine.ajax.view'))
                @slot('app_group',$group)
                @slot('ws_group','entity')
                @slot('ws_name','parameter-get')
                @slot('parameters', " 'parameter_id':parameter_id ")
                @slot('result_success')
                    ...
                    if (is_localized && !is_json) {
                        ...
                        if(valor.includes('image')){
                            ...
                            query = `{!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("input_localized","value",$lang.'form.edit.lbl_value',true,"col-md-12") !!}`;
                        }
                        $('#result').html(query);
        ...
        function updatePhotos(){
            ...
            @component(config($group.'.ui.component.engine.ajax.view'))
                ...
                @slot('result_success')
                    ...
                    ShowSuccessMessage("{{trans($lang.'form.msg_title_success')}}","{{trans($lang.'form.msg_description_success')}}");
                @endslot
                @slot('result_error')
                    ShowFormErrors(null,null,response,[]);
                    HideFullLoading();
        ...
        function uploadOrDeleteFile(action,dato,rename,num,lenguajes_headers){
            ...
            @component(config($group.'.ui.component.engine.ajax-internal_formdata.view'))
                @slot('app_group',$group)
                @slot('route','parameter-create-delete-card')
        ...
        function saveResult(id,resultado){
            ...
            @component(config($group.'.ui.component.engine.ajax.view'))
                @slot('app_group',$group)
                @slot('ws_group','entity')
                @slot('ws_name','parameter-register')
                @slot('parameters', " 'id':id, 'value':value ")
                @slot('result_success')
                    resetResults();
                    ShowSuccessMessage("{{trans($lang.'form.msg_title_success')}}","{{trans($lang.'form.msg_description_success')}}");
                @endslot
        ...
        function addSaveButton(){
            var query = `<button class="btn btn-success col-md-2" style="margin: 5px!important;" id="btnsave" onclick='getResult()'>{!! trans($lang.'btn_save') !!}</button>`;
            $('#div_btn').append(query);
        }
        function addAddButton(){
            var query = `<button class="btn btn-primary col-md-2" style="margin: 5px!important;" id="btnadd" onclick='addField()'>{!! trans($lang.'btn_add') !!}</button>`;
            $('#div_btn').prepend(query);
        }
        function addLoadFileButton(){
            var query = `<button class="btn btn-secondary col-md-2" style="margin: 5px!important;" id="btnLoadFiles" onclick='updatePhotos()'>{!! trans($lang.'btn_loadFiles') !!}</button>`;
            $('#div_btn').prepend(query);
        ...
        function chooseInput(name,id_value,cabezeras=null){
            if(name.includes('_color')){
                return `<input type="color" class="form-control" id="value" placeholder="{{trans($lang.'form.edit.lbl_value.placeholder')}}">`;
            }
            ...
            if(name.includes('image_parameter')){
                ...
                return `{!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("input_group","value",$lang.'form.edit.lbl_value',true,"col-md-12") !!}`;
            }
        }
