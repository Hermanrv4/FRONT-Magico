                  ...
                  <div class="form-group">
                      <div class="row">
                          <div class="row col-md-12">
                              {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("select","seleTypeGroup",$lang.'form.filters.lbl_company',true,"col-md-12","fas fa-envelope") !!}
                          </div>
                      </div>
                  </div>
                  <div class="row">
                      <div class="col-md-12" style="text-align: right!important;">
                          <button class="btn btn-success col-md-2" style="margin: 5px!important;" data-toggle="modal" data-target="#form_register_types" id="btnTypenew" >{!! trans($lang.'btn_register') !!}</button>
                      </div>
                  </div>
              </div>
...
@slot('modal_id', 'form_register_types')
...
@slot('modal_body')
        <div class="form-group">
            <div class="row col-md-12">
                ...
                {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("select","type_group_id",$lang.'form.filters.lbl_company',true,"col-md-12","fas fa-envelope") !!}
                {{-- select para el tipo de grupo --}}
                {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("input_group","code",$lang.'form.register.lbl_type_code',true,"col-sm-6") !!}
                {!! \App\Http\Modules\Admin\Services\HtmlService::ShowInput("input_localized","name",$lang.'form.register.lbl_type_name',true,"col-md-12") !!}
            </div>
        </div>
                <div class="row">
...
<script>
    ...
    document.getElementById("btnTypenew").addEventListener("click", ()=>{
        limpiarForm();
        $("#form_register_types b").html("{{strtoupper(trans($lang.'form.register.title'))}}");
        $("#btn_save_type").html("{{trans($lang.'form.register.btn_save_Type')}}");
    ...
    function SaveType(){
        ...
        @slot('app_group',$group)
                ...
                @slot('ws_name','type-register')
                @slot('parameters'," 'id':idtype, 'type_group_id': tip_group, 'name': name, 'code': code")
                @slot('result_success')
    ...
    function loadTypeId(id_type){
        ...
        @slot('app_group',$group)
                ...
                @slot('parameters', " 'type_id': id_type ")
                @slot('result_success')
                    $("#form_register_types").modal("show");
                    //editar formularios
                    $("#type_group_id").html("");
                    LoadDataForm(response.type_group_id);
                    $("#form_register_types-title").html("<b>{{trans($lang.'form.edit.title')}}<b>");
                    $("#btn_save_type").html("{{trans($lang.'form.register.btn_update_Type')}}");
                ...
                if ( $.fn.dataTable.isDataTable('#tbResults') ) {
                    ...
                    if(response.legth===0){
                        ...
                        $("#tbResults >thead").html("<tr>" +
                            "<th>{!! trans($lang.'result_table.col_type_id') !!}</th>"+
                            "<th>{!! trans($lang.'result_table.col_type_code') !!}</th>"+
                            "<th>{!! trans($lang.'result_table.col_type_group') !!}</th>"+
                            "<th>{!! trans($lang.'result_table.col_type_name') !!}</th>"+
                            "<th>{!! trans($lang.'result_table.col_options.title') !!}</th>"+
                        "</tr>");
                        for(let item of response){
                        document.getElementById("tablebody").innerHTML += `<tr> <td>${item.id}</td> <td>${item.code}</td> <td>${item.type_group_name_localized}</td> <td>${item.name_localized}</td> <td><div>
                            <button class="btn btn-sm btn-danger dropdown-toggle dropdown-icon tb-btn-group" onclick="" data-toggle='dropdown' style='z-index: 100!important;'> {!! trans($lang.'result_table.col_options.title') !!}<span class='sr-only'></span>
                                <div class='dropdown-menu' role='menu' x-placement='bottom-start'><a class='dropdown-item' href='#' onclick='loadTypeId(${item.id})'>{!! trans($lang.'result_table.col_options.edit') !!}</a><a class='dropdown-item' href='#' onclick='DeleteType(${item.id})'>{!! trans($lang.'result_table.col_options.delete') !!}</a></div>
                            </button>    
                        </div></td></tr>`;
                        }
                    }
    ...
    function LoadTypexGroup(group){
        ...
        @slot('app_group',$group)
                @slot('ws_group','entity')
                @slot('ws_name','type-get')
                @slot('parameters', " 'type_group_id': group ")
                @slot('result_success')
                if ( $.fn.dataTable.isDataTable('#tbResults') ) {
                        let table = $('#tbResults').DataTable();
                        table.destroy();
    ...
    function DeleteType(id_type){
        ...
        @slot('app_group',$group)
                @slot('ws_group','entity')
                @slot('ws_name','type-delete')
                @slot('parameters'," 'id': id_type ")
                @slot('result_success')
