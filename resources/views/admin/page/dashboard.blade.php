            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body">
                            ...
                            <a href='{{\App\Http\Common\Services\RouteService::GetAdminURL("products-list")}}' class="card-link">Ir</a>
                        </div>
                    </div>

                    {{-- <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h5 class="m-0">Ubicaciones</h5>
                        </div>
                        <div class="card-body">
                            <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                            <a href="{{\App\Http\Common\Services\RouteService::GetAdminURL("ubication-list")}}" class="card-link">Ir</a>
                        </div>
                    </div> --}}
                    <!-- /.card -->
                </div>
                ...
                <div class="col-lg-6">
                    {{-- <div class="card card-primary card-outline">
                        ...
                        <div class="card-body">
                            <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                            <a href="{{\App\Http\Common\Services\RouteService::GetAdminURL("customers-list")}}" class="card-link">Ir</a>
                        </div>
                    </div> --}}
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                            <a href="{{\App\Http\Common\Services\RouteService::GetAdminURL("sales-list")}}" class="card-link">Ir</a>
                        </div>
                    </div>
                </div>
                <!-- /.col-md-6 -->
...
@section('bottom_scripts')
    ...
    <script type="application/javascript">
        ...
        var last_monday=parseInt("{{$monday_last}}");
        ...
        function getSuscriberToday(fec_start, fec_end){
            @component(config($group.'.ui.component.engine.ajax.view'))
                    ...
                    @slot('ws_name','suscriber-get-date')
        ...
        function getMondayCode(){
            ...
            @component(config($group.'.ui.component.engine.ajax.view'))
                @slot('app_group',$group)
                    @slot('ws_group','entity')
                    @slot('ws_name','parameter-get-codes-all')
                    @slot('parameters', " 'code': code_monday ")
                    @slot('result_success')
                        console.log(response);
        ...
        function getOrderDateOfStatus(fec_start, fec_end){
            @component(config($group.'.ui.component.engine.ajax.view'))
                    ...
                    @slot('parameters', " 'date_start':fec_start , 'date_end':fec_end, 'id_status':2 ")
                    @slot('result_success')
        ...
        function getCustomerDateOfRegister(fec_start, fec_end=null){
            ...
            @component(config($group.'.ui.component.engine.ajax.view'))
                    @slot('app_group',$group)
                    @slot('ws_group','entity')
                    @slot('ws_name','user-get-register-of-date')
                    @slot('parameters', " 'date_start':fec_start , 'date_end':fec_end, 'id_status':2 ")
                    @slot('result_success')
                        console.log(response);
        ...
        function saveLasMonday(value){
            @component(config($group.'.ui.component.engine.ajax.view'))
                @slot('app_group',$group)
                    ...
                    @slot('parameters', " 'id': id_last_monday, 'value':value ")
                    @slot('result_success')
                        console.log(response);
                    @endslot
                @slot('result_error')
