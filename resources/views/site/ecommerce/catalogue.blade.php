...
@section('top_scripts')
    ...
    <link rel="stylesheet" type="text/css"
          href="{{asset("resources/assets/".$group."/ecommerce/cs/css/ion.rangeSlider.css")}}">
    <link rel="stylesheet" type="text/css"
          href="{{asset("resources/assets/".$group."/ecommerce/cs/css/ion.rangeSlider.css")}}">
    <link rel="stylesheet" type="text/css"
          href="{{asset("resources/assets/".$group."/ecommerce/cs/css/jquery-bar-rating.css")}}">
    <link rel="stylesheet" type="text/css"
          href="{{asset("resources/assets/".$group."/ecommerce/css/swiper.css")}}">
          <script
        src="{{asset("resources/assets/".$group."/ecommerce/js/swiper.js")}}"></script>
    <style>
    ...
    <script
        src="{{asset("resources/assets/".$group."/ecommerce/cs/js/ion.rangeSlider.js")}}"></script>
    <script
        src="{{asset("resources/assets/".$group."/ecommerce/cs/js/jquery-bar-rating.js")}}"></script>
@endsection
...
@section('body')
            ...
            {{--    @if($msg_discount_offer['active']==1)
                ...
                @if($msg_discount_offer3['active']==1)
                <script>
                </script>
                        <div class="cookie-section" style="padding: 1%;display: block;background: red;">
                            <span style="color: white;">{!! $msg_discount_offer3['text'] !!}<span style="
                            text-decoration:;
                            font-weight: bold;
                            color: white;">{!! $msg_discount_offer3['code'] !!}</span>
                            </span>
                    </div>
                @endif 
                    ...
                    <div class="product_sort">
                        <div class="classy-nav-container breakpoint-off">
                                <select id="order_by" onchange="ReloadWithSortBy();">
                                    <option
                                        value="0" disabled>{!! trans($lang.'lbl_sort_by') !!}</option>
                                    <option
                                        value="1" selected>{!! trans($lang.'lbl_sort_by_price_asc') !!}</option>
                                    <option
                                        value="2" >{!! trans($lang.'lbl_sort_by_price_dsc') !!}</option>
                                    <option
                                        value="3" >{!! trans($lang.'lbl_sort_by_name_asc') !!}</option>
...
@section('bottom_scripts')
    ...
    //DESCOMENTAR LANDING  DESCOMENTAR LANDING  DESCOMENTAR LANDING  DESCOMENTAR LANDING  DESCOMENTAR LANDING  DESCOMENTAR LANDING  
        ...
        function LoadProducts(){
            is_loading = true;
            let price_min = $('#price_min').text().replace('{{$data_filters["currency"]["symbol"]}}', '');
            let price_max = $('#price_max').text().replace('{{$data_filters["currency"]["symbol"]}}', '');
            ...
            let order_by = $('#order_by').val();
                ...
                @slot('route','product-catalogue')
                @slot('parameters',"categories: categories_list,
                    sale: is_sale,
                    search: txt_search,
                    page_num: page_num,
                    ...
                    let message = "{!! trans($lang.'lbl_counter') !!}";
                    ...
                    if (page_num == 1) {
                        if(response[1].length != 0){
                            ...
                            if(txt_search!="" && txt_search!="null"){
                                ...
                                $('#title_result_search').text("{!! $name_category !!}");
                            }
